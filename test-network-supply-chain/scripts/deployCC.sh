...
infoln "Installing chaincode on peer0.patient..."
installChaincode patient
infoln "Installing chaincode on peer0.doctor..."
installChaincode doctor

## query whether the chaincode is installed
queryInstalled patient
queryInstalled doctor

## approve the definition for all organizations and check
approveForMyOrg patient
approveForMyOrg doctor

commitChaincodeDefinition patient doctor

## query on all organizations to see that the definition committed successfully
queryCommitted patient
queryCommitted doctor

## Invoke the chaincode - this does require that the chaincode have the 'initLedger'
if [ "$CC_INIT_FCN" = "NA" ]; then
  infoln "Chaincode initialization is not required"
else
  chaincodeInvokeInit patient doctor
fi

chaincodeQuery patient

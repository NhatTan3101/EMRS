...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Patient Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-patient.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Doctor Identities"
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/doctor/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm compose_orderer.scm.com compose_peer0.patient.scm.com compose_peer0.doctor.scm.com 
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/patient/msp organizations/fabric-ca/patient/tls-cert.pem organizations/fabric-ca/patient/ca-cert.pem organizations/fabric-ca/patient/IssuerPublicKey organizations/fabric-ca/patient/IssuerRevocationPublicKey organizations/fabric-ca/patient/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/doctor/msp organizations/fabric-ca/doctor/tls-cert.pem organizations/fabric-ca/doctor/ca-cert.pem organizations/fabric-ca/doctor/IssuerPublicKey organizations/fabric-ca/doctor/IssuerRevocationPublicKey organizations/fabric-ca/doctor/fabric-ca-server.db'
